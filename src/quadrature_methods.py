...
class SamplingMethods:
    ...
    @staticmethod
    def gauss_legendre(l_max, n_theta=None, n_phi=None):
        """
        Gauss-Legendre quadrature on S2.
        Default: (l_max+1) GL nodes in cos(theta) and (2*l_max+2) uniform nodes in phi.
        Exact for polynomials up to degree 2*n_theta-1 in cos(theta).

        Args:
            l_max: maximum SH degree (used for default resolution)
            n_theta: override number of GL nodes in theta (default: l_max+1)
            n_phi: override number of uniform nodes in phi (default: 2*n_theta)
...
def get_sampling(method, l_max=None, resolution=None, degree=None, num_points=None,
    ...
    if method == 'uniform':
        ...
        pts, wts = SamplingMethods.gauss_legendre(l_max, n_theta=n_theta, n_phi=n_phi)
    elif method == 'lebedev':
